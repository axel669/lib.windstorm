import { component } from "#lib/macro"
import { preloadIcons } from "#lib/icon-processor"

preloadIcons(
    "square-dashed",
    "square-check-filled",
    "circle",
    "circle-check-filled",
    "circle-filled"
)

const core = `
    @color: var(--toggle-color, @primary);
    @size: 20px;

    @core-color: hsl(@color, @layer-element);

    pos: relative;
    w: @size;
    h: @size;
    appr: none;
    m: 0px;
    t.c: @core-color;
    v.a: middle;

    ! &::after {
        *content: "";
        pos: absolute;
        t.sz: calc(@size - 2px);
        *speak: none;
        t.st: normal;
        t.wt: 400;
        t.var: normal;
        t.tf: none;
        y: 50%;
        x: 50%;
        w: @size;
        h: @size;
        tf: translate(-50%, -50%);
        disp: flex;
        fl.cross: center;
        fl.main: center;
        over: hidden;
    }
`
component(":where(input[data-ws][type=checkbox]:not([switch]))")`
    ${core}

    ! &::after {
        font: ws-icon-square-dashed;
        *content: "square-dashed";
    }
    ! &:checked::after {
        font: ws-icon-square-check-filled;
        *content: "square-check-filled";
    }
`
component(":where(input[data-ws][type=radio])")`
    ${core}

    ! &::after {
        font: ws-icon-circle;
        *content: "circle";
    }
    ! &:checked::after {
        font: ws-icon-circle-check-filled;
        *content: "circle-check-filled";
    }
`
component(":where(input[data-ws][type=checkbox][switch])")`
    @color: @primary;
    @size: 20px;
    @anim-time: 100ms;

    appr: none;
    m: 0px;
    pos: relative;
    w: calc(@size * 2);
    h: @size;
    v.a: middle;

    ! &::before {
        *content: "";
        pos: absolute;
        inset: calc(@size / 4);
        b: 1px solid @color;
        r: calc(@size / 2);
        bg.c: hsl(@color, @layer-element);
        o: 0.5;
        tr: background-color @anim-time linear;
    }

    ! &::after {
        *content: "";
        pos: absolute;
        w: @size;
        h: @size;
        y: 0px;
        x: 0px;
        tr: left @anim-time linear, background-color @anim-time linear;
        bg.c: hsl(@mono, @layer-element);
        r: @size;
    }
    ! &:checked::after {
        x: @size;
        bg.c: hsl(@color, @layer-element);
    }
`

component("label[data-ws][toggle]")`
    @color: @primary;
    @active: @mono;

    pos: relative;
    *user-select: none;
    over: hidden;
    b: 0px solid hsl(@color, @layer-border);
    bg.c: hsl(@active, @layer-container);
    r: @base-radius;
    p: 4px;

    disp: flex;
    fl.main: space-between;

    ! &:focus-within {
        @active: @color;
    }

    ! & > input {
        @color: inherit;
        ! &:focus {
            outln: none;
        }
    }

    ! &:has(:not(button):disabled) {
        *filter: saturate(50%) brightness(0.7);
    }
`
