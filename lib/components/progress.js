...
customElements.define(
    ...
    class WSProgress extends HTMLElement {
        ...
        set value(next) {
            ...
            this.setAttribute("value", next?.toString() ?? "")
        }
        set buffer(next) {
            if (next === false) {
                this.removeAttribute("buffer")
            }
            this.setAttribute("buffer", next?.toString() ?? "")
        }
        set min(next) {
            if (next === false) {
                this.removeAttribute("min")
            }
            this.setAttribute("min", next?.toString() ?? "")
        }
        set max(next) {
            if (next === false) {
                this.removeAttribute("max")
            }
            this.setAttribute("max", next?.toString() ?? "")
        }
        set busy(next) {
            if (next === false) {
                this.removeAttribute("busy")
            }
            this.setAttribute("busy", next)
        }
