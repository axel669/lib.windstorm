...
function peg$parse(input, options) {
  ...
  function peg$f3(selector, rules) {
    return { type: "alt", selector: selector.trim(), rules }
  }
  function peg$f4(name, value) {
  ...
  function peg$parseCSSName() {
    let s0, s1, s2;

    const key = peg$currPos * 8 + 4;
    const cached = peg$resultsCache[key];
  ...
  function peg$parseSubRule() {
    let s0, s1, s2, s3, s4, s5, s6, s7;

    const key = peg$currPos * 8 + 5;
    const cached = peg$resultsCache[key];
